...
(function (root, factory) {
    ...
    const DEFAULT_SETTINGS = {
        ...
        // 授權寫入 Google Calendar：預設 false＝唯讀（只申請唯讀 scope；同步只拉回改動、不推送不刪、清場只清本地）。
        // 月度做法：每月下載各導師日曆的 .ics 匯入作基準，之後只看差異——本機資料不可能改壞 Calendar。
        gcalWrite: false,
        // 同步時「兩邊都有資訊、又不一樣」以哪邊為準：'gcal'＝Google Calendar（預設）｜'local'＝本系統。
        // 只有一邊改了的一律跟那一邊（Calendar 拖了時間→本地跟；本地標了請假而 Calendar 沒填→寫回／提示填）。
        gcalConflict: 'gcal',
        ...
        receiptAuto: true,
        // 收款確認＝學費單發出後收到錢，在同一個對話回一句（學校實際用語）。
        // 佔位符仍可用：{month} {paid}實收 {method} {date} {payinfo}——想寫明金額就自行加回去。
        receiptMsg: '已收妥，謝謝你！確認以上課程。🙋🏻‍♀️',
    ...
    // 從學生／小組的 tutor／tutorLevel 推出導師名單（data.js 沒有 defaultTutors 時「從 data.js 重新載入名單」用）：
    // 名字按出現次序去重；tutorLevel 是「資深導師」才算資深，其他一律普通
    function tutorsFromRoster(students, groups) {
        const out = [];
        (students || []).concat(groups || []).forEach(function (x) {
            const name = x ? String(x.tutor || '').trim() : '';
            if (!name || out.some(function (t) { return t.name === name; })) return;
            out.push({ name: name, tier: x.tutorLevel === '資深導師' ? '資深導師' : '普通導師' });
        });
        return out;
    }
    ...
    // ===== 資料存放位置 =====
    // 預設是瀏覽器 localStorage：同一網址（127.0.0.1:5500）的所有資料夾共用同一份，開 A 資料夾會看到 B 資料夾上次的狀態。
    // 由 serve.cmd 啟動時，serve.js 會把本資料夾 local-state.json 的內容注進頁面（window.__GAC_FILE_STATE），
    // 這裡就改用「檔案後備存放」：讀寫都在記憶體的 Map，改動 250ms 內合併成一次 save()（PUT /__state 寫回本資料夾）。
    // 介面與 localStorage 相同（getItem／setItem／removeItem／key／length），createStore 與各頁直接沿用。
    function createFileBackedStorage(initial, save, opts) {
        opts = opts || {};
        var delay = opts.delay === undefined || opts.delay === null ? 250 : opts.delay;
        var m = new Map();
        Object.keys(initial || {}).forEach(function (k) { if (typeof initial[k] === 'string') m.set(k, initial[k]); });
        var timer = null, inflight = null, dirty = false, changedWhileSaving = false; // dirty＝有改動還沒寫回（寫回失敗也算）
        function snapshot() { var o = {}; m.forEach(function (v, k) { o[k] = v; }); return o; }
        function schedule() { dirty = true; if (inflight) changedWhileSaving = true; if (timer) clearTimeout(timer); timer = setTimeout(function () { timer = null; flush(); }, delay); }
        // 有未寫回的改動才寫；寫回進行中又有改動 → 寫完再寫一次；回傳的 promise 等到全部落地。失敗保留 dirty，下次 flush 重試
        function flush() {
            if (timer) { clearTimeout(timer); timer = null; }
            if (inflight) return inflight;
            if (!dirty) return Promise.resolve();
            dirty = false; changedWhileSaving = false;
            inflight = Promise.resolve().then(function () { return save(snapshot()); })
                .then(function () { api.lastError = null; }, function (e) { dirty = true; api.lastError = e; if (typeof api.onError === 'function') api.onError(e); })
                .then(function () { inflight = null; if (changedWhileSaving) { changedWhileSaving = false; return flush(); } }); // 失敗不自動重試（避免無限循環）：dirty 留著，下次改動或 flush 再寫
            return inflight;
        }
        var api = {
            isFileBacked: true,
            ...
            setItem: function (k, v) { m.set(String(k), String(v)); schedule(); },
            removeItem: function (k) { if (m.delete(String(k))) schedule(); },
            key: function (i) { var keys = Array.from(m.keys()); return i >= 0 && i < keys.length ? keys[i] : null; },
            get length() { return m.size; },
            hasPending: function () { return !!timer || !!inflight || dirty; },
            snapshot: snapshot,
            flush: flush,
            _m: m
        };
    ...
    // 預設的寫回：PUT /__state（serve.js 收到就寫 local-state.json）；本文小於 60KB 加 keepalive，關頁那一下也送得出
    function fileSaver(win) {
        return function (obj) {
            var body = JSON.stringify(obj);
            return win.fetch('/__state', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: body, keepalive: body.length < 60000 })
                .then(function (r) { if (!r.ok) throw new Error('HTTP ' + r.status); });
        };
    }

    // 頁面啟動時選存放位置：serve.js 注入了 __GAC_FILE_STATE → 檔案後備（並掛在 win.__GAC_FILE_STORAGE 供除錯／測試）；否則 localStorage
    function pickStorage(win) {
        var st = win && win.__GAC_FILE_STATE, cfg = win && win.__GAC_FILE_STORE;
        if (st && typeof st === 'object' && cfg) {
            var s = createFileBackedStorage(st, cfg.save || fileSaver(win), { delay: cfg.delay });
            win.__GAC_FILE_STORAGE = s;
            return s;
        }
    ...
    return {
        ...
        createFileBackedStorage: createFileBackedStorage,
        fileSaver: fileSaver,
        pickStorage: pickStorage,
        buildExportPayload: buildExportPayload,
        tutorsFromRoster: tutorsFromRoster,

...
(function (root, factory) {
    ...
    const DEFAULT_SETTINGS = {
        ...
        tplMove: '已確認 {fromDate} ({fromWeekday}) {fromTime} 的課堂改期至 {date} ({weekday}) {time}。'
    };
    ...
    function createStore(storageLike) {
        ...
        return {
            ...
            // 導師 {name, tier, calendarEmbed（Google 日曆嵌入連結或日曆 ID）, calendarId（API 讀取用）}
            loadTutors: function (defaults) {
                const norm = function (t) {
                    return {
                        name: String(t.name).trim(), tier: t.tier === '資深導師' ? '資深導師' : '普通導師',
                        calendarEmbed: String(t.calendarEmbed || '').trim(), calendarId: String(t.calendarId || '').trim()
                    };
                };

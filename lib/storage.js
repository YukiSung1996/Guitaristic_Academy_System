    ...
    function createStore(storageLike) {
        ...
        return {
            ...
            loadTutors: function (defaults) {
                const d = (defaults || []).map(function (t) { return { name: String(t.name), tier: t.tier === '資深導師' ? '資深導師' : '普通導師' }; });
                const raw = get(KEYS.tutors);
                const arr = safeParse(raw, '導師名單', null);
                if (Array.isArray(arr)) {
                    return arr.filter(function (t) { return t && String(t.name || '').trim(); })
                        .map(function (t) { return { name: String(t.name).trim(), tier: t.tier === '資深導師' ? '資深導師' : '普通導師' }; });
                }
                set(KEYS.tutors, d);
                return d;
            },

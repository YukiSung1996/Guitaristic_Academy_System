...
(function (root, factory) {
    ...
    function upsertTuition(log, params) {
        ...
        if (!existing) {
            log[key] = {
                ...
                paid: false, paidAmount: 0, payMethod: '', payDate: '', receipt: false, payNote: '', // paid＝推導快取，見 setPayment
                status: 'TODO',
                sentAt: null,
                method: null,
    ...
    // ===== 繳費紀錄（只限 TUITION 條目）=====
    // 「已繳」不是可以直接勾的欄位，而是推導：**實收金額 > 0 且 選了付款方式** 才算收到錢
    //（只填金額卻沒有付款方式＝不知道錢從哪來，一律當未繳）。e.paid 只是這個判斷的快取。
    // fields 可含：paidAmount（實收）、payMethod（'1'|'2'|…，名稱在設定）、payDate、receipt、payNote、
    //   clearPayment（true＝清掉實收與付款方式，回到未繳；日期保留以便查閱）。
    // 便利規則：選了付款方式而實收仍為 0 → 預設收足整額；成為已繳而日期空白 → 補今天。
    function paymentValid(e) {
        return !!e && Number(e.paidAmount) > 0 && String(e.payMethod || '') !== '';
    }
    ...
    function setPayment(log, key, fields, todayStr) {
        ...
        const f = fields || {};
        const wasPaid = paymentValid(e);
        if ('receipt' in f) e.receipt = !!f.receipt;
        if ('payNote' in f) e.payNote = String(f.payNote || '');
        if ('payDate' in f) e.payDate = String(f.payDate || '');
        if (f.clearPayment) { e.paidAmount = 0; e.payMethod = ''; }
        if ('payMethod' in f) {
            e.payMethod = String(f.payMethod || '');
            if (e.payMethod && !(Number(e.paidAmount) > 0)) e.paidAmount = Number(e.amount) || 0; // 最常見：一次過付清
        }
        if ('paidAmount' in f) e.paidAmount = Math.max(0, Number(f.paidAmount) || 0);
        e.paid = paymentValid(e);
        if (e.paid && !e.payDate) e.payDate = todayStr || '';
        if (wasPaid && !e.paid) e.receiptSkipped = false; // 回到未繳＝新一輪：之後再繳清會重新建立收款確認
        return e;
    }

    // 'paid'（實收 ≥ 應收）| 'partial'（0 < 實收 < 應收）| 'unpaid'（含「填了金額但沒選付款方式」）
    function paymentStatus(e) {
        if (!paymentValid(e)) return 'unpaid';
    ...
    function paymentTotals(entries) {
        ...
        (entries || []).forEach(function (e) {
            ...
            if (paymentValid(e)) paid += Number(e.paidAmount) || 0;
        });
    ...
    function paymentVars(t, opts) {
        ...
        const got = paymentValid(t) ? (Number(t.paidAmount) || 0) : 0;
        return {

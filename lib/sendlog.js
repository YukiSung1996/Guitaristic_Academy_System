...
(function (root, factory) {
    ...
    function upsertTuition(log, params) {
        ...
        if (!existing) {
            log[key] = {
                ...
                // 繳費紀錄（對應手工學費表：Check?／Paid／Method／Date／Receipt），由 setPayment 維護
                checked: false, paid: false, paidAmount: 0, payMethod: '', payDate: '', receipt: false, payNote: '',
                status: 'TODO',
                sentAt: null,
                method: null,
    ...
    // ===== 繳費紀錄（只限 TUITION 條目）=====
    // fields 可含：checked（已核對金額）、paid（已繳）、paidAmount（實收）、payMethod（'1'|'2'|'3'，名稱在設定）、
    ...
    function setPayment(log, key, fields, todayStr) {
        ...
        const f = fields || {};
        if ('checked' in f) e.checked = !!f.checked;

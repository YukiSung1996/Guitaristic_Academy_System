...
(function (root, factory) {
    ...
    // ===== 費率覆寫（設定頁改價）：鍵 tier|program|level|classType|duration；基準價首次套用時記在 row.baseRate =====
    function overrideKey(row) {
        return [row.tutor, row.instrument, row.grade, row.classType, Number(row.duration)].join('|');
    }

    // 就地套用：有有效覆寫的列 rate＝覆寫值，其餘回到基準價。回傳套用筆數。
    function applyOverrides(table, overrides) {
        var n = 0;
        (table || []).forEach(function (row) {
            if (row.baseRate === undefined) row.baseRate = row.rate;
            var v = overrides ? overrides[overrideKey(row)] : undefined;
            if (v !== undefined && v !== null && v !== '' && !isNaN(Number(v))) { row.rate = Number(v); n++; }
            else row.rate = row.baseRate;
        });
        return n;
    }

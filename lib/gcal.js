...
(function (root, factory) {
    ...
    //   opts = { students:[{id,name}], groups:[{id,name}], tutor?: 事件來自此導師的日曆——只有「Calendar 沒有這堂」限他的課；配對不分導師,
    ...
    function reconcileByContent(lessons, events, opts) {
        ...
        // 配對不分導師：事件寫明了學號／小組名，就算放在別位導師的日曆（新建頁預設放主日曆）也認得是那一節；
        // 「Calendar 沒有這堂」才限本導師的課（這本日曆只涵蓋他的課）
        var allCells = GACSchedule.groupByCell(lessons || []).filter(function (c) { return !skip[c.key]; });
        var cells = allCells.filter(function (c) { return !o.tutor || c.lessons[0].tutor === o.tutor; });
        var cellsByTarget = {};
    ...
    function createRestClient(deps) {
        ...
        // 回傳時間用這個時區（本系統取牆鐘部分）：不帶的話 Google 用日曆本身的時區，日曆設成別的時區會整批偏移
        var tzParam = deps.timeZone ? '&timeZone=' + encodeURIComponent(deps.timeZone) : '';
        ...
        return {
            ...
            listWindow: function (timeMin, timeMax) {
                ...
                function page(pageToken) {
                    var url = base + '?maxResults=2500&singleEvents=true' + tzParam +

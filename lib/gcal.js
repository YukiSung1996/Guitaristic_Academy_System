...
(function (root, factory) {
    ...
    // 本地「有資訊」：請假 L/SL/TL、缺席 NS、已上課 ATT（Calendar 沒有對應碼）；已排課＝沒資訊（null）
    function localStatusCode(lesson) {
        if (lesson.status === 'LEAVE') return lesson.leaveType || 'L';
        if (lesson.status === 'NOSHOW') return 'NS';
        if (lesson.status === 'ATTENDED') return 'ATT';
        return null;
    }
    ...
    // Calendar 事件的碼：L/SL/TL/NS；沒填／MU → null
    function calStatusCode(event) {
        var to = parseStatusCode(event);
        return to ? (to.status === 'NOSHOW' ? 'NS' : to.leaveType) : null;
    }
    ...
    //   toLocal [{cell, lessons, lesson, event, to}]    Calendar 的碼 → 本地（與 reconcile.statusChanges 同形）
    //   toGcal  [{cell, lessons, lesson, event, code}]  本地的狀態 → 寫到 Calendar（code ''＝清掉碼，如已上課）
    // 只有一邊有資訊 → 流向沒資訊的那邊；兩邊都有、相同 → 不動；兩邊都有、不同 → 看 rule。
    // 小組成員狀態不一（MIXED）：Calendar 為準時照舊逐個成員更新；本系統為準時一個碼表達不了 → 不動。
    // 已上課（ATT）：Calendar 沒碼 → 不動；Calendar 有碼且本系統為準 → 清掉 Calendar 的碼。
    function planStatusSync(pairs, rule) {
        ...
        (pairs || []).forEach(function (p) {
            ...
            if (cal === null) {
                if (local && local !== 'ATT' && local !== 'MIXED') toGcal.push({ cell: cell, lessons: cell.lessons, lesson: rep, event: ev, code: local });
                return;
            }
            if (local === cal) return;
            ...
            toGcal.push({ cell: cell, lessons: cell.lessons, lesson: rep, event: ev, code: local === 'ATT' ? '' : local });
        });
    ...
    function statusPatchPayload(cell, code, event) {
        var f = cell.lessons[0];
        var location = code || (f.isMakeup ? 'MU' : '');
        var desc = withStatusLine(event && event.description, location);

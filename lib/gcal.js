...
(function (root, factory) {
    ...
    function noteLocalMove(lesson, snap) {
        ...
        mf.inCal = mf.inCal || !!snap.gcalEventId || !!snap.gcalAdded;
    ...
    function planLocalMoves(lessons, events) {
        ...
        var moves = [], settled = [], used = {};
        cells.forEach(function (cell) {
            var flagged = cell.lessons.filter(function (l) { return l.gcalMovedFrom; });
            if (!flagged.length) return;
            var mf = flagged[0].gcalMovedFrom;
            var ev = byKey[cell.key], at;
            if (ev) {
                at = eventStartToLocal(ev);
                if (at && at.date === cell.date && at.time === cell.time) settled.push({ cell: cell, lessons: cell.lessons, event: ev });
                else if (at && at.date === mf.date && at.time === mf.time && !used[ev.id]) {
                    used[ev.id] = true;
                    moves.push({ cell: cell, lessons: cell.lessons, lesson: cell.lessons[0], event: ev, from: at, tagOnly: false });
                }
                return;
            }
            var cand = (mf.key && !localKeys[mf.key] && byKey[mf.key]) || null;
            if (!cand) flagged.some(function (l) {
                var e = l.gcalEventId && byId[l.gcalEventId];
                var k = e && eventCellKey(e);
                if (e && !(k && localKeys[k])) { cand = e; return true; }
                return false;
            });
            if (!cand || used[cand.id]) return;
            used[cand.id] = true;
            at = eventStartToLocal(cand) || {};
            moves.push({ cell: cell, lessons: cell.lessons, lesson: cell.lessons[0], event: cand, from: at,
                tagOnly: at.date === cell.date && at.time === cell.time });
        });
        return { moves: moves, settled: settled };
    }
    ...
    function reconcile(lessons, events, knownStudentIds) {
        ...
        cells.forEach(function (cell) {
            ...
            if (ev) { matchedKeys[cell.key] = true; if (ev.id) matchedEventIds[ev.id] = true; }
            if (!ev) {
        ...
        return { timeChanges: timeChanges, deletions: deletions, statusChanges: statusChanges, manualNew: manualNew,
            matchedKeys: matchedKeys, matchedEventIds: matchedEventIds };
    }
    ...
    function reconcileByContent(lessons, events, opts) {
        ...
        var staleMoves = [], settled = [], staleUsed = {};
        ...
        return { timeChanges: timeChanges, deletions: deletions, statusChanges: statusChanges, manualNew: manualNew, unmatched: unmatched,
            staleMoves: staleMoves, settled: settled, matched: Object.keys(paired).length };
    }

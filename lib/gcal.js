...
(function (root, factory) {
    ...
    // A＝出席（已上課）。留空也算出席：課的時間過了、「狀態：」還是空的，同步時就當已上課（planStatusSync／reconcileByContent 帶 now）
    ...
    // 這節已經結束了嗎（now：Date）：牆鐘時間比，結束＝開始＋時長。沒給 now → 一律 false（純函式不偷看時鐘）
    function lessonEnded(lesson, now) {
        if (!now || !lesson || !lesson.date || !lesson.time) return false;
        var n = now instanceof Date ? now : new Date(now);
        if (isNaN(n.getTime())) return false;
        var nowKey = n.getFullYear() + '-' + pad2(n.getMonth() + 1) + '-' + pad2(n.getDate()) + 'T' + pad2(n.getHours()) + ':' + pad2(n.getMinutes()) + ':00';
        return endDateTime(lesson.date, lesson.time, lesson.duration) <= nowKey;
    }
    ...
    // planStatusSync(pairs, rule, opts)：pairs [{cell, event}]（已配對的課節與事件）；rule 'gcal'｜'local'（兩邊都有資訊且不同時以哪邊為準）
    //   toLocal [{cell, lessons, lesson, event, to, blank?}]  Calendar 的碼 → 本地（與 reconcile.statusChanges 同形；blank＝留空而課已結束 → 出席）
    //   toGcal  [{cell, lessons, lesson, event, code}]         本地的狀態 → 寫到 Calendar（A＝出席）
    // 只有一邊有資訊 → 流向沒資訊的那邊；兩邊都有、相同 → 不動；兩邊都有、不同 → 看 rule。
    // 小組成員狀態不一（MIXED）：Calendar 為準時照舊逐個成員更新；本系統為準時一個碼表達不了 → 不動。
    // 已上課（A）與 Calendar 留空視為同一回事：Calendar 沒碼 → 不動；Calendar 有別的碼且本系統為準 → 寫回 A。
    // 留空＝已上課：opts.now 有給、全員仍是已排課、而這節已結束 → 拉回出席（標 blank，面板另列一組）。
    function planStatusSync(pairs, rule, opts) {
        ...
        (pairs || []).forEach(function (p) {
            ...
            if (cal === null) {
                if (local === null && lessonEnded(rep, now)) toLocal.push({ cell: cell, lessons: cell.lessons, lesson: rep, event: ev, to: { status: 'ATTENDED', leaveType: '' }, blank: true });
                else if (local && local !== 'A' && local !== 'MIXED') toGcal.push({ cell: cell, lessons: cell.lessons, lesson: rep, event: ev, code: local });
                return;
            }
            if (local === cal) return;
            if (local === null || rule !== 'local') {
                ...
                if (differs) toLocal.push({ cell: cell, lessons: cell.lessons, lesson: rep, event: ev, to: to });
                return;
            }
            if (local === 'MIXED') return;
            toGcal.push({ cell: cell, lessons: cell.lessons, lesson: rep, event: ev, code: local });
        });
    ...
    function reconcileByContent(lessons, events, opts) {
        ...
        (events || []).forEach(function (ev) {
            ...
            if (to) {
                ...
                if (differs) statusChanges.push({ cell: cell, lessons: cell.lessons, lesson: rep, event: ev, to: to });
            } else if (o.now && cellStatusCode(cell) === null && lessonEnded(rep, o.now)) {
                // 留空＝已上課：全員仍是已排課、課已結束 → 出席
                statusChanges.push({ cell: cell, lessons: cell.lessons, lesson: rep, event: ev, to: { status: 'ATTENDED', leaveType: '' }, blank: true });
            }
        });

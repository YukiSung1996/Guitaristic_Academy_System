...
(function (root, factory) {
    ...
    // events：已篩成「帶 gacLessonId 標籤」的事件（cancelled 在此再擋一次）；lessons：全部本地課（跨月）。
    // 回傳 existing（已導入）、stale [{lesson,event,reasons[]}]（時間/標題/狀態碼與本地不符）、
    //      orphans [event]（標籤對不上任何本地課＝改動/重新生成後的殘留）。
    function importPrecheck(lessons, events, opts) {
        var byId = {};
        (lessons || []).forEach(function (l) { byId[l.lessonId] = l; });
        ...
        (events || []).forEach(function (ev) {
            ...
            existing.push(ev);
            var lesson = byId[id];
            if (!lesson) { orphans.push(ev); return; }
            var expected = lessonToEventPayload(lesson, opts);
            var local = eventStartToLocal(ev);
            var reasons = [];
            if (!local || local.date !== lesson.date || local.time !== lesson.time) reasons.push('時間');
            if (String(ev.summary || '') !== expected.summary) reasons.push('標題');
            if (String(ev.location || '').trim() !== expected.location) reasons.push('狀態碼');
            if (reasons.length) stale.push({ lesson: lesson, event: ev, reasons: reasons });
        });
    ...
    // client 介面：remove(eventId)→Promise
    function deleteEvents(client, items) {
